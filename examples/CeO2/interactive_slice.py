...
def create_interactive_slice_plot(elf_data, span_vectors):
    """Create an interactive plot with sliders for slicing the ELF data."""
    ...
    for z in range(elf_shape[2]):
        fig.add_trace(go.Heatmap(
            ...
            x0=0, dx=span_vectors[0, 0] / elf_shape[0],
            y0=0, dy=span_vectors[1, 1] / elf_shape[1]
        ))
    ...
    for x in range(elf_shape[0]):
        fig.add_trace(go.Heatmap(
            ...
            x0=0, dx=span_vectors[1, 1] / elf_shape[1],
            y0=0, dy=span_vectors[2, 2] / elf_shape[2]
        ))
    ...
    for y in range(elf_shape[1]):
        fig.add_trace(go.Heatmap(
            ...
            x0=0, dx=span_vectors[2, 2] / elf_shape[2],
            y0=0, dy=span_vectors[0, 0] / elf_shape[0]
        ))
    ...
    for i in range(elf_shape[0]):
        ...
        step["args"][0]["visible"][elf_shape[2] + i] = True  # Toggle i-th trace to be visible
    ...
    for i in range(elf_shape[1]):
        ...
        step["args"][0]["visible"][elf_shape[2] + elf_shape[0] + i] = True  # Toggle i-th trace to be visible
    ...
    fig.update_layout(
        updatemenus=[
            dict(
                ...
                buttons=[
                    dict(
                        args=[{"visible": [False] * len(fig.data)}, {"sliders": [sliders[0]]}],
                        label="XY Plane",
                        method="update"
                    ),
                    dict(
                        args=[{"visible": [False] * len(fig.data)}, {"sliders": [sliders[1]]}],
                        label="YZ Plane",
                        method="update"
                    ),
                    dict(
                        args=[{"visible": [False] * len(fig.data)}, {"sliders": [sliders[2]]}],
                        label="ZX Plane",
                        method="update"
                    )
                ],
...
def main():
    ...
    elf_data, origin, span_vectors, atoms = read_elf_data_xsf('wan_elf.xsf')  # Ensure this file exists
    ...
    create_interactive_slice_plot(elf_data, span_vectors)
