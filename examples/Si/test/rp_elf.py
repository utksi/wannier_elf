...
def periodic_gradient(f, dx):
    ...
    for i in range(3):
        grad[i] = (np.roll(padded, -1, axis=i) - np.roll(padded, 1, axis=i)) / (2 * dx[i])
    return grad[:, pad_width:-pad_width, pad_width:-pad_width, pad_width:-pad_width]
...
def periodic_laplacian(f, dx):
    ...
    for i in range(3):
        laplacian += (np.roll(padded, -1, axis=i) - 2*padded + np.roll(padded, 1, axis=i)) / dx[i]**2
    return laplacian[pad_width:-pad_width, pad_width:-pad_width, pad_width:-pad_width]
...
def calculate_ke_density(wannier_data, span_vectors, electron_density, use_fft=True):
    ...
    dV = np.prod(dx)  # Volume element in Bohr^3

    if use_fft:
        # Generate k-point grid in reciprocal space
        kx = 2 * np.pi * np.fft.fftfreq(grid_shape[0], dx[0])
        ky = 2 * np.pi * np.fft.fftfreq(grid_shape[1], dx[1])
        kz = 2 * np.pi * np.fft.fftfreq(grid_shape[2], dx[2])
...
if __name__ == "__main__":
    ...
    try:
        ...
        output_xsf = f"elf_{'fft' if use_fft else 'real'}_{('norm' if normalize else 'nonorm')}_{('interp' if interpolate else 'nointerp')}_{('mask' if mask else 'nomask')}.xsf"
