...
def pbc_pad(data, pad_width):
    padded = np.pad(data, pad_width, mode='wrap')
    return padded
...
def calculate_ke_density(wannier_data, span_vectors, electron_density):
    ...
    # Smooth Wannier functions
    smoothed_wannier_data = np.array([gaussian_filter(wf.real, sigma=0.1) + 1j * gaussian_filter(wf.imag, sigma=0.1) for wf in wannier_data])
    
    laplacian_term = np.zeros_like(electron_density)
    for wf in smoothed_wannier_data:
...
def calculate_elf(electron_density, ke_density, normalize=False, threshold=1e-6):
    ...
    elf = 1 / (1 + (ke_density / np.maximum(tf_ke_density, threshold))**2)
